...
void unit_Model_add( void){
    ...
    vector <Stock*> stocks1 = {source1, destiny1};
    vector <Flow*> flows1 = {flu1};

    assert(m1->getStocks() == stocks1);

    assert(m1->getFlows() == flows1);
    ...
    Flow * flu2 = m2->createFlow(source2, destiny2, 1, exponencial, true);
...
void unit_Model_run(void){
    ...
    m->createFlow(source, destiny, 0, logistical, false);

...
int pesquisaArvoreB (TipoRegistro *x, TipoApontador ap, int *contComp, int *contTrans){
    ...
    if(x->chave < ap->r[i - 1].chave) 
        pesquisaArvoreB(x, ap->p[i - 1], contComp, contTrans); //Chamo recursivamente para esquerda se a chave desejada for menor que a chave da arvore

    else 
        pesquisaArvoreB(x, ap->p[i], contComp, contTrans); //Chamo recursivamente para direita se a chave desejada for maior que a chave da arvore
...
//Calcula o tempo total de toda execução, juntando pre-processamento e pesquisa
void calcularTempoTotal(double tPreProce, double tPesquisa, double *tTotal) {

    *tTotal = tPesquisa + tPreProce; 
}

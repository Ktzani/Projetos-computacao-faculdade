...
void intercala2fcaminhos(FILE *arquivo, int quantidadeDeAlunos, int *contLeitura, int *contEscrita, int *contComp){
    ...
    //Leio 20 registros a cada iteração do for do arquivo principal, coloco esses 20 registros em um vetor e os 
    //ordeno com um insertion sort. Logo, em seguida coloco esse vetor ordenado que será um bloco em cada um das fitas 
    //de entrada
    ...
    alocaciaBlocos(FitasEntrada, FitasSaida, divisaoNaoExata);
...
void intercalacaoPropriamenteDita (FILE *arquivo, Fita *FitasEntrada, Fita *FitasSaida, Aluno *vec, int quantidadeDeAlunos, int *contLeitura, int *contEscrita, int *contComp){
    ...
    while (numerototalLidas < quantidadeDeAlunos){ 
        ...
        } while(contadorDoWhile < REGISTROS);
...
void alocaciaBlocos(Fita *FitasEntrada, Fita *FitasSaida, int divisaoNaoExata){
    ...
    for(i = 0; i < FITAS; i++){
        ...
        FitasSaida[i].numReg = alocaBlocos(FitasEntrada[0].qtdBlocos, FitasSaida[i].numReg);
    ...
    if(divisaoNaoExata == 1){
        ...
        divisaoNaoExataFunc(&FitasEntrada[5]);
    }
}
...
void divisaoNaoExataFunc (Fita *fita6){
    ...
    fita6->numReg[fita6->qtdBlocos - 1] = 5;
}
